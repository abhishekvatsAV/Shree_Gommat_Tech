    ...
    fields.append(
        {
            NAME_KEY: YEAR_TOTAL,
            TYPE_KEY: "INTEGER",
            CALCULATION_KEY: "SUM(" + YEAR_TOTAL + ")",
        }
    )  # year total column
    for key in sorted(MONTHS):  # month columns
...
def get_cost_fields_list(report_type: str) -> Sequence[Dict[str, Any]]:
    ...
    fields.append(
        {
            NAME_KEY: name_field[NAME_KEY],
            TYPE_KEY: name_field[TYPE_KEY],
            OPTIONS_KEY: name_field[OPTIONS_KEY],
        }
    )
    for field in ALL_REPORT_FIELDS:  # fields in all reports
...
def create_cost_table_sql_texts(report_type: str) -> str:
    ...
    for field in report_fields:
        ...
        fields_and_options.append(field_text)
        if (
            field[NAME_KEY] in COSTS_KEY_FIELDS
            or field[NAME_KEY] == name_field[NAME_KEY]
        ):
            key_fields.append(field[NAME_KEY])
    sql_text += "\n\t" + ", \n\t".join(fields_and_options)
...
def update_vendor_in_all_tables(old_name: str, new_name: str):
    ...
    for cost_table in REPORT_TYPE_SWITCHER.keys():
        sql_text, data = update_vendor_name_sql_text(
            cost_table + COST_TABLE_SUFFIX, old_name, new_name
        )
        sql_texts.append({"sql_text": sql_text, "data": data})
...
def replace_costs_sql_text(
    report_type: str, data: Sequence[Dict[str, Any]]
) -> Tuple[str, Sequence[Sequence[Any]]]:
...
def delete_costs_sql_text(
    report_type: str, vendor: str, month: int, year: int, name: str
) -> Tuple[str, Sequence[Sequence[Any]]]:
...
def cost_chart_search_sql_text(
    report: str,
    vendor: str,
    name: str,
    metric_type: str,
...
def get_names_with_costs_sql_text(
    report: str, vendor: str, start_year: int, end_year: int
) -> Tuple[str, Sequence[Any]]:
...
def get_costs_sql_text(
    report_type: str, vendor: str = None, name: str = None
) -> Tuple[str, Sequence[Any]]:
    ...
    sql_text = (
        get_sql_select_statement(
            [name_field, "vendor"]
            + [field[NAME_KEY] for field in COST_FIELDS]
            + ["year", "month"],
...
def setup_database(drop_tables: bool, emit_signal: bool = True):
    ...
    sql_texts.update(
        {
            report_type + COST_TABLE_SUFFIX: create_cost_table_sql_texts(report_type)
...
def first_time_setup():
    ...
    if not os.path.exists(DATABASE_FOLDER):
        if ManageDBSettingsHandler.settings.show_debug_messages:
            print("CREATE " + DATABASE_FOLDER)
        os.makedirs(DATABASE_FOLDER)
    if not os.path.exists(DATABASE_LOCATION):
        if ManageDBSettingsHandler.settings.show_debug_messages:
            print("CREATE " + DATABASE_LOCATION)
        setup_database(False)
    if not os.path.exists(COSTS_SAVE_FOLDER):
        if ManageDBSettingsHandler.settings.show_debug_messages:
            print("CREATE " + COSTS_SAVE_FOLDER)
